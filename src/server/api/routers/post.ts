...
export const postRouter = createTRPCRouter({
  ...
  getIdsByName: publicProcedure.input(z.object(
    {
        playingWith: z.string().min(2).regex(/^(?!\s).+(?<!\s)$/gm),
    ...
    const playingWith = await ctx.db.champion.findUnique({where: {name: input.playingWith}})
    const playingAgainst= await ctx.db.champion.findUnique({where: {name: input.playingAgainst}})
    const rune = await ctx.db.rune.findUnique({where: {name: input.rune}})
    const mainSummoner = await ctx.db.summonerSpell.findUnique({where: {name: input.mainSummoner}})
    const secondarySummoner = await ctx.db.summonerSpell.findUnique({where: {name: input.secondarySummoner}})
    return{
      playingWith,
      playingAgainst,
      rune,
  ...
  //   const res = await fetch("http://ddragon.leagueoflegends.com/cdn/13.20.1/data/en_US/item.json")
  //   const data = await res.json() as {data: Record<string, unknown>}
  ...
  createPost: publicProcedure.input(z.object(
    ...
    getDataPostsById: publicProcedure.input(z.number().min(1)).query(async ({ ctx, input }) => {
      ...
      for(const post of postsData){
        const itemData = []
        const itemsIds = await ctx.db.itemsOnPost.findMany({where: {postId: post.id}})
        for(const item of itemsIds){
          const itemRawData = await ctx.db.item.findUnique({where: {id: item.itemId}})
        ...
        const entirePostInfo = {
          ...
          items: itemData,
          playingWith: await ctx.db.champion.findUnique({where: {id: post.playingWithId}}),
          playingAgainst: await ctx.db.champion.findUnique({where: {id: post.playingAgainstId}}),
          mainSummoner: await ctx.db.summonerSpell.findUnique({where: {id: post.mainSummonerSpellId}}),
          secondarySummoner: await ctx.db.summonerSpell.findUnique({where: {id: post.secondarySummonerSpellId}})
        }
    ...
    getPostById: publicProcedure.input(z.number()).query(async ({ ctx, input }) => {
      const post = await ctx.db.post.findUnique({where: {id: input}})
      const playingAgainst = await ctx.db.champion.findUnique({where: {id: post?.playingAgainstId}})
      const abilities = await ctx.db.skill.findMany({where: {championId: post?.playingAgainstId}})
      const itemsIds = await ctx.db.itemsOnPost.findMany({where: {postId: post?.id}})
      const items = []
      for(const itemData of itemsIds){
        const item = await ctx.db.item.findUnique({where: {id: itemData.itemId}})
        items.push(item)
      }
      const rune = await ctx.db.rune.findUnique({where: {id: post?.runeId}})
      const mainSummoner = await ctx.db.summonerSpell.findUnique({where: {id: post?.mainSummonerSpellId}})
      const secondarySummoner = await ctx.db.summonerSpell.findUnique({where: {id: post?.secondarySummonerSpellId}})
      const postData = {
